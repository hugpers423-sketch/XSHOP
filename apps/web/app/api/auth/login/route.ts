...
export async function POST(request: Request) {
  ...
  if (!body || typeof body.email !== 'string' || !isValidEmail(body.email) || typeof body.password !== 'string') {
    return NextResponse.json({ error: 'Datos inválidos' }, { status: 400 });
  }

  try {
    const response = await fetch(`${AUTH_API_URL}/api/users/login`, {
      method: 'POST',
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.error || 'Credenciales incorrectas' }, { status: response.status });
    }
